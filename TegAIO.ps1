...
try{
$adgroup2 = Get-ADGroup -Identity $group2 -ErrorAction Stop} catch {Write-Warning "Cannot find group for $group2"
}
try{
...
If ($adgroup2) {
    Write-Host "Processing $($adgroup2.SamAccountName)" -ForegroundColor Green
    Add-ADGroupMember -Identity $group2 -Members $adcomputer
    }
If ($adgroup3) {
...
Copy-Item '\\uselycspzzzfs05\Elyria IT Share$\ultraVNC\*' -Destination 'X:\temp'
